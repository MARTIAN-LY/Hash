...
import com.example.hash.databinding.FragmentComplainBinding
import com.example.hash.home.adapter.BlogRecyclerAdapter
import com.example.hash.home.models.DataSource3
...
class ComplainFragment : Fragment() {
    ...
    private fun initRecycler(){
        ...
        binding.complainRecycler.apply {
            layoutManager = LinearLayoutManager(activity)
            addItemDecoration(topPadding)
            blogAdapter = BlogRecyclerAdapter(DataSource3.createDataSet())
            adapter = blogAdapter
        }
    }
